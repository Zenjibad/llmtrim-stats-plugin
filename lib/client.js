...
function apply(ctx) {
	...
	function Dashboard() {
		...
		}), "Static")), h("div", { className: "lts-config-stats" }, STAT_KEYS.map((key) => h("label", { key }, h("input", {
			type: "checkbox",
	...
	function Carousel() {
		...
		const pool = cfg.mode === "static" && cfg.staticStats.length > 0 ? all.filter((s) => cfg.staticStats.includes(s.key)) : all;
