window.__ModuleLoader__.load({ id: "llmtrim-stats-plugin", factory: (require) => {
"use strict";
	...
	return [
		{
			key: "savedToday",
			label: "Saved today",
			value: fmtUsd(m?.savedTodayUsd)
		},
		{
			key: "savedTotal",
			label: "Saved total",
			value: fmtUsd(m?.savedUsd)
		},
		{
			key: "youPaid",
			label: "You paid",
			value: fmtUsd(m?.paidUsd)
		},
		{
			key: "wouldHave",
			label: "Would have cost",
			value: fmtUsd(m?.wouldHaveUsd)
		},
		{
			key: "savedWeek",
			label: "Saved this week",
			value: fmtUsd(m?.savedWeekUsd)
		},
		{
			key: "tokensTrimmed",
			label: "Tokens trimmed",
			value: fmtTokens(t?.tokensTrimmed ?? 0)
		},
		{
			key: "requests",
			label: "Requests",
		...
		},
		{
			key: "inputSavedPct",
			label: "Input saved",
			value: fmtPct(t?.inputSavedPct)
		},
		{
			key: "roundTripPct",
			label: "Round-trip",
			value: fmtPct(c?.roundTripPct)
		}
	];
}
function apply(ctx) {
	...
	function useSnapshot() {
		...
		const reload = react.default.useCallback(() => {
			void fetch(API, { cache: "no-store" }).then((res) => res.json()).then((snap) => {
				setData(snap);
				setFailed(!snap.ok);
	...
	function Dashboard() {
		...
		const [savedAt, setSavedAt] = react.default.useState(0);
		const [saveErr, setSaveErr] = react.default.useState(null);
		react.default.useEffect(() => {
			if (savedAt === 0) return;
			const t = setTimeout(() => setSavedAt(0), 2500);
			return () => clearTimeout(t);
		}, [savedAt]);
		if (failed && (data === null || !data.ok)) return h("div", { className: "lts-section" }, h("div", { className: "lts-failure" }, h("p", { role: "alert" }, "llmtrim stats are unavailable."), h("button", {
			type: "button",
			onClick: reload
		}, "Retry")));
		...
		const onMode = (mode) => {
			const next = {
				mode,
				staticStats: cfg.staticStats.length > 0 ? cfg.staticStats : [...STAT_KEYS]
			};
		...
		return h("div", { className: "lts-section" }, h("div", { className: "lts-heading" }, h("h3", null, "llmtrim savings"), d?.daemon ? h("span", {
			className: "lts-badge",
			"data-ok": String(d.daemon.running)
		}, d.daemon.running ? "daemon healthy" : "daemon stopped") : null, h("span", null, d?.daemon?.version ?? "")), h("div", { className: "lts-cards" }, h("div", { className: "lts-card" }, h("div", { className: "lts-card-label" }, "You paid"), h("div", { className: "lts-card-value" }, stats.youPaid), h("div", { className: "lts-card-sub" }, "total billed")), h("div", { className: "lts-card" }, h("div", { className: "lts-card-label" }, "Would have cost"), h("div", { className: "lts-card-value" }, stats.wouldHave), h("div", { className: "lts-card-sub" }, "uncompressed")), h("div", { className: "lts-card" }, h("div", { className: "lts-card-label" }, "Saved today"), h("div", { className: "lts-card-value" }, stats.savedToday), h("div", { className: "lts-card-sub" }, "per-turn ledger")), h("div", { className: "lts-card" }, h("div", { className: "lts-card-label" }, "Saved this week"), h("div", { className: "lts-card-value" }, stats.savedWeek), h("div", { className: "lts-card-sub" }, "prorated by tokens")), h("div", { className: "lts-card" }, h("div", { className: "lts-card-label" }, "Tokens trimmed"), h("div", { className: "lts-card-value" }, stats.tokensTrimmed), h("div", { className: "lts-card-sub" }, fmtPct(d?.totals?.inputSavedPct) + " input")), h("div", { className: "lts-card" }, h("div", { className: "lts-card-label" }, "Requests"), h("div", { className: "lts-card-value" }, stats.requests), h("div", { className: "lts-card-sub" }, fmtPct(d?.totals?.outputSavedPct) + " output")), h("div", { className: "lts-card" }, h("div", { className: "lts-card-label" }, "Net saved (re-priced)"), h("div", { className: "lts-card-value" }, fmtUsd(d?.cost?.netSavedUsd)), h("div", { className: "lts-card-sub" }, fmtPct(d?.cost?.roundTripPct) + " round-trip"))), h("div", { className: "lts-heading" }, h("h3", null, "Carousel"), saveErr ? h("span", { className: "lts-save-err" }, saveErr) : null, savedAt > 0 ? h("span", { className: "lts-saved" }, "Saved ✓") : null), h("div", { className: "lts-config" }, h("div", { className: "lts-config-row" }, h("label", null, h("input", {
			type: "radio",
			name: "lts-mode",
			checked: cfg.mode === "rotating",
			onChange: () => onMode("rotating")
		}), "Rotating"), h("label", null, h("input", {
			type: "radio",
			name: "lts-mode",
			checked: cfg.mode === "static",
			onChange: () => onMode("static")
		}), "Static")), h("div", { className: "lts-config-hint" }, cfg.mode === "rotating" ? "Rotating cycles one stat at a time through the selected stats." : "Static shows every selected stat at once, without rotating."), h("div", { className: "lts-config-stats" }, STAT_KEYS.map((key) => h("label", { key }, h("input", {
			type: "checkbox",
			checked: cfg.staticStats.includes(key),
			onChange: () => onToggleStat(key)
		}), statLabels[key])))), h("div", { className: "lts-heading" }, h("h3", null, "Per model"), h("span", null, String(d?.byModel?.length ?? 0))), h("table", { className: "lts-table" }, h("thead", null, h("tr", null, h("th", null, "Model"), h("th", null, "Requests"), h("th", null, "Saved"), h("th", null, "USD"))), h("tbody", null, (d?.byModel ?? []).length === 0 ? h("tr", null, h("td", { colSpan: 4 }, "No model data yet.")) : (d?.byModel ?? []).map((m) => h("tr", { key: m.model }, h("td", null, m.model), h("td", null, String(m.requests)), h("td", null, fmtPct(m.savedPct)), h("td", null, fmtUsd(m.costSavedUsd)))))));
	}
	/** Composer dock: rotating or static stats strip. */
	function Carousel() {
		const { data, failed } = useSnapshot();
		const [idx, setIdx] = react.default.useState(0);
		react.default.useEffect(() => {
		...
		if (cfg.mode === "static") return react.default.createElement("div", { className: "lts-dock lts-dock-static" }, pool.map((s) => react.default.createElement("span", {
			className: "lts-dock-static-item",
			key: s.key
		}, react.default.createElement("span", {
			className: "lts-dock-dot",
			"data-ok": String(d.daemon?.running ?? false)
		}), react.default.createElement("span", { className: "lts-dock-key" }, s.label), react.default.createElement("span", { className: "lts-dock-value" }, s.value))));
		const slide = pool[idx % pool.length] ?? pool[0];
		if (slide === undefined) return null;
		return react.default.createElement("div", {
			className: "lts-dock",
			key: slide.key + String(idx)
		}, react.default.createElement("span", {
			className: "lts-dock-dot",
			"data-ok": String(d.daemon?.running ?? false)
		}), react.default.createElement("span", { className: "lts-dock-key" }, slide.label), react.default.createElement("span", { className: "lts-dock-value lts-carousel-enter" }, slide.value));
	}
	...
	slots.inject("conversation.composer.dock", () => slots.register({
		...
		id: "llmtrim-carousel",
		order: 15
	}, () => react.default.createElement(Carousel)));
}
