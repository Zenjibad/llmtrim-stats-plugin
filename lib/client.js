...
function apply(ctx) {
	...
	function Dashboard() {
		...
		}, d.daemon.running ? "daemon healthy" : "daemon stopped") : null, h("span", null, d?.daemon?.version ?? "")), h("div", { className: "lts-cards" }, h("div", { className: "lts-card" }, h("div", { className: "lts-card-label" }, "Saved (proxy bills)"), h("div", { className: "lts-card-value" }, d?.money ? fmtUsd(d.money.savedUsd) : "—"), h("div", { className: "lts-card-sub" }, d?.money ? "today " + fmtUsd(d.money.savedTodayUsd) : "")), h("div", { className: "lts-card" }, h("div", { className: "lts-card-label" }, "Tokens trimmed"), h("div", { className: "lts-card-value" }, d?.totals ? fmtTokens(d.totals.tokensTrimmed) : "—"), h("div", { className: "lts-card-sub" }, d?.totals ? fmtPct(d.totals.inputSavedPct) + " input" : "")), h("div", { className: "lts-card" }, h("div", { className: "lts-card-label" }, "Requests"), h("div", { className: "lts-card-value" }, d?.totals ? String(d.totals.requests) : "—"), h("div", { className: "lts-card-sub" }, d?.totals ? fmtPct(d.totals.outputSavedPct) + " output" : "")), h("div", { className: "lts-card" }, h("div", { className: "lts-card-label" }, "Net saved (re-priced)"), h("div", { className: "lts-card-value" }, d?.cost ? fmtUsd(d.cost.netSavedUsd) : "—"), h("div", { className: "lts-card-sub" }, d?.cost ? fmtPct(d.cost.roundTripPct) + " round-trip" : ""))), h("div", { className: "lts-heading" }, h("h3", null, "Per model"), h("span", null, String(d?.byModel?.length ?? 0))), h("table", { className: "lts-table" }, h("thead", null, h("tr", null, h("th", null, "Model"), h("th", null, "Requests"), h("th", null, "Saved"), h("th", null, "USD"))), h("tbody", null, (d?.byModel ?? []).length === 0 ? h("tr", null, h("td", { colSpan: 4 }, "No model data yet.")) : (d?.byModel ?? []).map((m) => h("tr", { key: m.model }, h("td", null, m.model), h("td", null, String(m.requests)), h("td", null, fmtPct(m.savedPct)), h("td", null, fmtUsd(m.costSavedUsd)))))));
	}
	...
	function Carousel() {
		...
		const slides = [
			{
				key: "Saved today",
				value: fmtUsd(d.money.savedTodayUsd)
			},
			{
				key: "Saved total",
				value: fmtUsd(d.money.savedUsd)
			},
			{
				key: "Tokens trimmed",
				value: fmtTokens(d.totals.tokensTrimmed)
			},
			{
				key: "Requests",
				value: String(d.totals.requests)
			},
			{
				key: "Input saved",
				value: fmtPct(d.totals.inputSavedPct)
			},
			{
				key: "Round-trip",
				value: fmtPct(d.cost?.roundTripPct)
			}
		];
		...
		}), react.default.createElement("span", { className: "lts-dock-key" }, slide.key), react.default.createElement("span", { className: "lts-dock-value lts-carousel-enter" }, slide.value));
	}
