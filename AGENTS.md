...
| `package.json` | `exports["./client"]`, `dsh.bundle.patch`, `dsh.client` (`platform: 'web'`, inject edges), peers react + @deepseek-ai/cordis, dep @deepseek-ai/schemastery. |
...
4. **Keep the snapshot shape stable**: the client reads `daemon`, `totals` (requests, tokensTrimmed, inputSavedPct, outputSavedPct), `money` (savedUsd, savedTodayUsd, paidUsd, wouldHaveUsd, savedWeekUsd), `cost` (netSavedUsd, roundTripPct), `byModel`, `config` (`{mode, staticStats}` from the settings namespace). If llmtrim's `status --json` keys change, update the reshape in `src/index.ts` and keep the interface in sync.
...
- **Add a carousel stat option**: add the key to `STAT_KEYS` in `src/index.ts` (schema default + `BASE_CONFIG`) and `src/client/index.tsx` (client `STAT_KEYS` + `statLabels` + `buildSlides`), keeping both lists in sync.
...
- Distinguishing traits: packaged profile plugin (persists across restarts), host pulls the same `llmtrim status --json` the CLI uses (no ledger parsing), settings dashboard with four money cards (paid / would-have / today / prorated week) + configurable rotating-or-static composer carousel (persisted `{mode, staticStats}` settings namespace), host HTTP routes (`GET` api + `PUT` config) instead of dynamic RPC, real-Node host half, pure `--dsw-alias-*` theming.
